...
const PodcastCard = ({ podcast, user, setUser,showDeleteButton }) => {
  const [favourite, setFavourite] = useState(false);
  const [videoOpen, setVideoOpen] = useState(false);
  const { loguser, fetchSession } = useSession();
  ...
  const favoritpodcast = async () => {
    ...
    const updatedUser = user ? { ...user, favorites: [...user.favorites, podcast] } : { favorites: [podcast] };
    setUser(updatedUser);
  ...
  const unfavoritpodcast = () => {
    ...
    const updatedFavorites = user.favorites.filter((fav) => fav.v_id !== podcast.v_id);
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res = await axios.delete(`http://localhost:8080/delfev/${podcast.v_id}`);
          ...
          message.error("An error occurred while removing the podcast from favorites");
        }
      }
    });
  };
  ...
  const deletePodcast = () => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res = await axios.delete(`http://localhost:8080/delvid/${podcast.v_id}`);
          if (res.data.status === "Success") {
            message.success('Podcast deleted successfully');
          //   setUser(prevUser => (
          //     {
          //     ...prevUser,
          //     // podcasts: prevUser.favorites.filter(p => p.v_id !== podcast.v_id)
          //   }
          // ));
            // localStorage.setItem('user', JSON.stringify({
            //   ...user,
            //   // podcasts: user.favorites.filter(p => p.v_id !== podcast.v_id)
            // }));
          } else {
  ...
  const handleviewupdate = async (v_id, views) => {
    try {
      const res = await axios.put('http://localhost:8080/update_views', { views: views + 1, v_id: v_id });
      if (res.data.status === "Success") {
  ...
  return (
    <Card>
      <Top>
        <Favorite>
          {favourite ? (
            <FavoriteIcon style={{ color: "#E30022", width: '16px', height: '16px' }} onClick={unfavoritpodcast} />
          ) : (
            <FavoriteIcon style={{ width: '16px', height: '16px' }} onClick={favoritpodcast} />
          )}
        </Favorite>
        {showDeleteButton && (
        ...
        <CardImage src={podcast.thubnail ? `${baseUrl}/${podcast.thubnail}` : "https://t3.ftcdn.net/jpg/03/89/66/40/360_F_389664003_euiB7gaAQ0zMM74c0GBV9fOgeHqM0GZU.jpg"} />
      </Top>
      <CardInformation>
        <MainInfo>
      ...
      <PlayIcon>
        {"video" === 'video' ? (
          <PlayArrowIcon onClick={() => { setVideoOpen(true); handleviewupdate(podcast.v_id, podcast.views); }} style={{ width: '28px', height: '28px' }} />
        ) : (
          <HeadphonesIcon style={{ width: '28px', height: '28px' }} />
        )}
      </PlayIcon>
      <VideoPlayer open={videoOpen} setOpen={setVideoOpen} thumbnail={podcast.thubnail} path={podcast.path} />
    </Card>
  );
};
