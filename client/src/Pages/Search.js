...
const Search = () => {
    ...
    const filterCategory = async(value) => {
        if(value.trim().length === 0){
            ...
            setFilterCategories(Category.filter(cat => cat.name.toLowerCase().includes(value)))
        }
      ...
      <SearchMain>
          ...
          <Categories>
                ...
                {filterCategories.map((category) => (
                    <Link to={`/Showpodcasts/${category.c_id}`} style={{ textDecoration: "none" }} key={category.c_id}>
                 ...
                 <Topic>Videos</Topic>
                     <DivPodcasts>
                      {videos && videos.map(video => (
                       <PodcastCard key={video.id} podcast={video} user={user} setUser={setUser}/>
                        ))}
                     </DivPodcasts>
